 ...
 * Xcode **8+** and all its additional components installed (will need to open and run any new version of Xcode)
 * `git` (from Xcode Command Line Tools or `brew install git`)

## Usage

Please note: munki_rebrand must be run as root in order to successfully build the output pkg. You will need Xcode (8+) and its command-line tools installed. **It is important to open and run any newly installed version of Xcode and ensure its additional components are installed before trying to run munki-rebrand, or the build may fail.**

At its simplest you can use ```sudo ./munki_rebrand.py --appname "Amazing Software Center"``` to rename Managed Software Center to Amazing Software Center in the Finder where the language is set to English (U.S.) and you install Munki with the outputted pkg file. Use the ```--localized``` option to also changed all localized translations of "Managed Software Center" to your desired app name.
